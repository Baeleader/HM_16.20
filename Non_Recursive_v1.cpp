// [TS] 64x64 함수
#if AMP_ENC_SPEEDUP
Void TEncCu::xCompressCUSS(TComDataCU*& rpcBestCU, TComDataCU*& rpcTempCU, const UInt uiDepth DEBUG_STRING_FN_DECLARE(sDebug_), PartSize eParentPartSize)
#else
Void TEncCu::xCompressCUSS(TComDataCU*& rpcBestCU, TComDataCU*& rpcTempCU, const UInt uiDepth)
#endif
{
...
// [TS] 32x32 함수
#if AMP_ENC_SPEEDUP
Void TEncCu::xCompressCUTT(TComDataCU * &rpcBestCU, TComDataCU * &rpcTempCU, const UInt uiDepth DEBUG_STRING_FN_DECLARE(sDebug_), PartSize eParentPartSize)
#else
Void TEncCu::xCompressCUTT(TComDataCU * &rpcBestCU, TComDataCU * &rpcTempCU, const UInt uiDepth)
#endif
{
    TComPic* pcPic = rpcBestCU->getPic();
    DEBUG_STRING_NEW(sDebug)
    ...
    if (bSubBranch && uiDepth < sps.getLog2DiffMaxMinCodingBlockSize() && (!getFastDeltaQp() || uiWidth > fastDeltaQPCuMaxSize || bBoundary)) // [TS] split check
    {
        ...
        for (Int iQP = iMinQP; iQP <= iMaxQP; iQP++)
        {
            ...
            DEBUG_STRING_NEW(sTempDebug)
                ...
                for (UInt uiDepTwoPartIdx = 0; uiDepTwoPartIdx < 4; uiDepTwoPartIdx++)// [TS] NextDepth부분을 현재의 Depth로 바꿔주면은 된다.
                {
                    pcSubBestPartCU->initSubCU(rpcTempCU, uiDepTwoPartIdx, uhNextDepth, iQP);           // clear sub partition datas or init. 하위파티션 데이터 초기화 혹은 지우는 부분
                    pcSubTempPartCU->initSubCU(rpcTempCU, uiDepTwoPartIdx, uhNextDepth, iQP);           // clear sub partition datas or init.

                    if ((pcSubBestPartCU->getCUPelX() < sps.getPicWidthInLumaSamples()) && (pcSubBestPartCU->getCUPelY() < sps.getPicHeightInLumaSamples()))
                    {
                        if (0 == uiDepTwoPartIdx) //initialize RD with previous depth buffer 이전 뎁스 버퍼 사용 RD초기화
                        {
                        ...
                        // [TS] 16x16들어가기
                        xCompressCUOO(pcSubBestPartCU, pcSubTempPartCU, uhNextDepth);

                        rpcTempCU->copyPartFrom(pcSubBestPartCU, uiDepTwoPartIdx, uhNextDepth);         // Keep best part data to current temporary data. 현재 임시 데이터에 대한 Best Part data 보관
                        xCopyYuv2Tmp(pcSubBestPartCU->getTotalNumPart() * uiDepTwoPartIdx, uhNextDepth);
                    ...
                    else
                    {
                        ...
                        rpcTempCU->copyPartFrom(pcSubBestPartCU, uiDepTwoPartIdx, uhNextDepth);
                    }
                }
...
// [TS] 16x16 함수
#if AMP_ENC_SPEEDUP
Void TEncCu::xCompressCUOO(TComDataCU * &rpcBestCU, TComDataCU * &rpcTempCU, const UInt uiDepth DEBUG_STRING_FN_DECLARE(sDebug_), PartSize eParentPartSize)
#else
Void TEncCu::xCompressCUOO(TComDataCU * &rpcBestCU, TComDataCU * &rpcTempCU, const UInt uiDepth)
#endif
{
    TComPic* pcPic = rpcBestCU->getPic();
    DEBUG_STRING_NEW(sDebug)
    ...
    if (bSubBranch && uiDepth < sps.getLog2DiffMaxMinCodingBlockSize() && (!getFastDeltaQp() || uiWidth > fastDeltaQPCuMaxSize || bBoundary)) // [TS] Recursive 구조 탈출 조건문
    {
        //further split
        Double splitTotalCost = 0;
        ...
        for (Int iQP = iMinQP; iQP <= iMaxQP; iQP++)
        {
            ...
            DEBUG_STRING_NEW(sTempDebug)
                ...
                for (UInt uiDepThrPartIdx = 0; uiDepThrPartIdx < 4; uiDepThrPartIdx++)// [TS] NextDepth부분을 현재의 Depth로 바꿔주면은 된다.
                {
                    pcSubBestPartCU->initSubCU(rpcTempCU, uiDepThrPartIdx, uhNextDepth, iQP);           // clear sub partition datas or init. 하위파티션 데이터 초기화 혹은 지우는 부분
                    pcSubTempPartCU->initSubCU(rpcTempCU, uiDepThrPartIdx, uhNextDepth, iQP);           // clear sub partition datas or init.

                    if ((pcSubBestPartCU->getCUPelX() < sps.getPicWidthInLumaSamples()) && (pcSubBestPartCU->getCUPelY() < sps.getPicHeightInLumaSamples()))
                    {
                        if (0 == uiDepThrPartIdx) //initialize RD with previous depth buffer 이전 뎁스 버퍼 사용 RD초기화
                        {
                            m_pppcRDSbacCoder[uhNextDepth][CI_CURR_BEST]->load(m_pppcRDSbacCoder[uiDepth][CI_CURR_BEST]);
                        }
                        else
                        ...
                        // [TS] 8x8들어가기
                        xCompressCUEE(pcSubBestPartCU, pcSubTempPartCU, uhNextDepth);

                        rpcTempCU->copyPartFrom(pcSubBestPartCU, uiDepThrPartIdx, uhNextDepth);         // Keep best part data to current temporary data. 현재 임시 데이터에 대한 Best Part data 보관
                        xCopyYuv2Tmp(pcSubBestPartCU->getTotalNumPart() * uiDepThrPartIdx, uhNextDepth);
                        if (m_pcEncCfg->getLumaLevelToDeltaQPMapping().isEnabled() && pps.getMaxCuDQPDepth() >= 1)
                        {
                            splitTotalCost += pcSubBestPartCU->getTotalCost();
                        }
                    ...
                    {
                        pcSubBestPartCU->copyToPic(uhNextDepth);
                        rpcTempCU->copyPartFrom(pcSubBestPartCU, uiDepThrPartIdx, uhNextDepth);
                    }
                }
...
// [TS] 8x8 함수
#if AMP_ENC_SPEEDUP
Void TEncCu::xCompressCUEE(TComDataCU * &rpcBestCU, TComDataCU * &rpcTempCU, const UInt uiDepth DEBUG_STRING_FN_DECLARE(sDebug_), PartSize eParentPartSize)
#else
Void TEncCu::xCompressCUEE(TComDataCU * &rpcBestCU, TComDataCU * &rpcTempCU, const UInt uiDepth)
#endif
{
    TComPic* pcPic = rpcBestCU->getPic();
    DEBUG_STRING_NEW(sDebug)
    ...
    //const UInt uiEEWidth = rpcBestCU->getWidth(0) >> 1;
    //const UInt uiEEHeight = rpcBestCU->getHeight(0) >> 1;
...
/*xCompressCUSS(m_ppcBestCU[0], m_ppcTempCU[0], 0 DEBUG_STRING_PASS_INTO(sDebug));
for (UInt uiDepOnePartIdx = 0; uiDepOnePartIdx < 4; uiDepOnePartIdx++)
{
    xCompressCUTT(m_ppcBestCU[0], m_ppcTempCU[0], uiOneDepth DEBUG_STRING_PASS_INTO(sDebug), uiDepOnePartIdx);

    for (UInt uiDepTwoPartIdx = 0; uiDepTwoPartIdx < 4; uiDepTwoPartIdx++)
    {
        xCompressCUOO(m_ppcBestCU[0], m_ppcTempCU[0], uiTwoDepth DEBUG_STRING_PASS_INTO(sDebug), uiDepTwoPartIdx);

        for (UInt uiDepThrPartIdx = 0; uiDepThrPartIdx < 4; uiDepThrPartIdx++)
        {
            xCompressCUEE(m_ppcBestCU[0], m_ppcTempCU[0], uiThreeDepth DEBUG_STRING_PASS_INTO(sDebug), uiDepThrPartIdx);
        }
        xCheckBestMode(rpcBestCU, rpcTempCU, uiTwoDepth);
    }
    xCheckBestMode(rpcBestCU, rpcTempCU, uiOneDepth);
}
...
#if SS_COMPRESSCU
// [TS] 64x64 ~ 8x8함수
Void TEncCu::NonRecursiveSSEE(TComDataCU * &rpcBestCU, TComDataCU * &rpcTempCU, const UInt uiDepth)
{
    TComPic* pcPic = rpcBestCU->getPic();
    DEBUG_STRING_NEW(sDebug)
        const TComPPS& pps = *(rpcTempCU->getSlice()->getPPS());
